"""
Agent Scheduler — 定期的に記事を自動生成し、GitHub Pagesに公開する常駐プロセス

フロー:
  1. Groq で記事生成 → PostgreSQL 保存
  2. publisher.py が Hugo markdown を GitHub API で push
...
async def main():
    ...
    interval_minutes = int(float(os.getenv("ARTICLE_GENERATION_INTERVAL_HOURS", "4")) * 60)
    ...
    scheduler.start()
    print(f"[Scheduler] 起動完了 — {interval_minutes}分ごとに記事生成")
